...
class FitOutCashoutValueController extends Controller
{
    public function storeFoCashoutMonthlyValue(Request $request, $projectId)
    {
        $request->validate([
        ...
        $monthlyValueRecord = FoCashoutMonthlyBase::firstOrCreate(
    ...
    public function calculateFoCashoutCumulativeValue($projectId) {
        ...
        $monthlyReports = FoCashoutMonthlyBase::where('projectId', $projectId)
...
public function getCurrentFoCashoutMonthValue(Request $request, $projectId)
{
    // Define month abbreviation mapping
    $monthMapping = [
        'January' => 'jan',
    ...
    $monthlyValueRecord = FoCashoutMonthlyBase::where('ProjectId', $projectId)
                                         ->where('year', $currentYear)
                                         ->first();
