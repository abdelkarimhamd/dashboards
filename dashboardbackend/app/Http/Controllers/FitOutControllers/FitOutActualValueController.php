...
use Carbon\Carbon;
class FitOutActualValueController extends Controller
{
    public function storeFoActualMonthlyValue(Request $request, $projectId)
    {
        $request->validate([
        ...
        $monthlyValueRecord = FoActualMonthlyBase::firstOrCreate(
    ...
    public function calculateFoActualCumulativeValue($projectId) {
        ...
        $monthlyReports = FoActualMonthlyBase::where('projectId', $projectId)
...
public function getCurrentFoActualMonthValue(Request $request, $projectId)
{
    // Define month abbreviation mapping
    $monthMapping = [
        'January' => 'jan',
    ...
    $monthlyValueRecord = FoActualMonthlyBase::where('ProjectId', $projectId)
                                         ->where('year', $currentYear)
                                         ->first();
