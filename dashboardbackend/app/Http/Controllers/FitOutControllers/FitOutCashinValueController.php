...
class FitOutCashinValueController extends Controller
{
    public function storeFoCashinMonthlyValue(Request $request, $projectId)
    {
        $request->validate([
        ...
        $monthlyValueRecord = FoCashinMonthlyBase::firstOrCreate(
    ...
    public function calculateFoCashinCumulativeValue($projectId) {
        ...
        $monthlyReports = FoCashinMonthlyBase::where('projectId', $projectId)
...
public function getCurrentFoCashinMonthValue(Request $request, $projectId)
{
    // Define month abbreviation mapping
    $monthMapping = [
        'January' => 'jan',
    ...
    $monthlyValueRecord = FoCashinMonthlyBase::where('ProjectId', $projectId)
                                         ->where('year', $currentYear)
                                         ->first();
