...
use Carbon\Carbon;
use App\Mail\EscalationActualInvoiceMail;
use App\Mail\FinalActualInvoiceEscalationMail;
use App\Mail\ReminderActualInvoiceMail;
use Illuminate\Support\Facades\Log;

class SendActualInvoiceProjectReminderEmails extends Command
{
    protected $signature = 'send:project-reminder-emails';
    protected $description = 'Send email reminders to project managers if actual invoice is not filled';
    ...
    public function handle()
{
    ...
    foreach ($projects as $project) {
        ...
        $actualValue = ActualValueMonthlyBase::where('projectId', $project->id)
        ...
        if ($actualValue && isset($actualValue->{$currentMonthAbbreviation}) && $actualValue->{$currentMonthAbbreviation} == 0) {
            ...
            if ($daysSinceStartOfMonth == 5) {
                Mail::to($projectManager->email)->send(new ReminderActualInvoiceMail($project));
            } elseif ($daysSinceStartOfMonth == 8) {
                Mail::to($projectManager->email)->send(new EscalationActualInvoiceMail($project));
                ...
                Mail::to($projectManager->email)
                    ...
                    ->send(new FinalActualInvoiceEscalationMail($project));
            }
        }
    }
}
